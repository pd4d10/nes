...
class PPU {
  ...
  reset() {
    ...
    // Memory
    this.vramMem = new List(0x8000);
    this.spriteMem = new List(0x100);
    ...
    this.f_bgPatternTable =
        0; // Background Pattern Table address. 0=0x0000,1=0x1000
    this.f_spPatternTable =
        0; // Sprite Pattern Table address. 0=0x0000,1=0x1000
    this.f_addrInc = 0; // PPU Address Increment. 0=1,1=32
    this.f_nTblAddress =
        0; // Name Table Address. 0=0x2000,1=0x2400,2=0x2800,3=0x2C00

    // Control Flags Register 2:
    this.f_color = 0; // Background color. 0=black, 1=blue, 2=green, 4=red
    this.f_spVisibility = 0; // Sprite visibility. 0=not displayed,1=displayed
    this.f_bgVisibility =
        0; // Background visibility. 0=Not Displayed,1=displayed
    this.f_spClipping =
        0; // Sprite clipping. 0=Sprites invisible in left 8-pixel column,1=No clipping
    this.f_bgClipping =
        0; // Background clipping. 0=BG invisible in left 8-pixel column, 1=No clipping
    ...
    this.attrib = new List(32);
    this.buffer = new List(256 * 240);
    this.bgbuffer = new List(256 * 240);
    this.pixrendered = new List(256 * 240);

    this.validTileData = null;

    this.scantile = new List(32);
    ...
    // Sprite data:
    this.sprX = new List(64); // X coordinate
    this.sprY = new List(64); // Y coordinate
    this.sprTile = new List(64); // Tile Index (into pattern table)
    this.sprCol = new List(64); // Upper two bits of color
    this.vertFlip = new List(64); // Vertical Flip
    this.horiFlip = new List(64); // Horizontal Flip
    this.bgPriority = new List(64); // Background priority
    this.spr0HitX = 0; // Sprite #0 hit X coordinate
    this.spr0HitY = 0; // Sprite #0 hit Y coordinate
    this.hitSpr0 = false;

    // Palette data:
    this.sprPalette = new List(16);
    this.imgPalette = new List(16);

    // Create pattern table tile buffers:
    this.ptTile = new List(512);
    for (i = 0; i < 512; i++) {
    ...
    // Name table data:
    this.ntable1 = new List(4);
    this.currentMirroring = -1;
    this.nameTable = new List(4);
    for (i = 0; i < 4; i++) {
      this.nameTable[i] = new NameTable(32, 32, "Nt" + i.toString());
    }

    // Initialize mirroring lookup table:
    this.vramMirrorTable = new List(0x8000);
    for (i = 0; i < 0x8000; i++) {
  ...
  mirroredWrite(address, value) {
    if (address >= 0x3f00 && address < 0x3f20) {
      ...
      if (address < this.vramMirrorTable.length) {
        ...
        throw "Invalid VRAM address: " + address.toString(16);
      }
    }
  }
  ...
  triggerRendering() {
    if (this.scanline >= 21 && this.scanline <= 260) {
      ...
      this.renderFramePartially(this.lastRenderedScanline + 1,
  ...
  updatePalettes() {
    ...
    for (i = 0; i < 16; i++) {
      if (this.f_dispType == 0) {
        this.imgPalette[i] =
            this.palTable.getEntry(this.vramMem[0x3f00 + i] & 63);
      } else {
        this.imgPalette[i] =
            this.palTable.getEntry(this.vramMem[0x3f00 + i] & 32);
      }
    }
    for (i = 0; i < 16; i++) {
      if (this.f_dispType == 0) {
        this.sprPalette[i] =
            this.palTable.getEntry(this.vramMem[0x3f10 + i] & 63);
      } else {
        this.sprPalette[i] =
            this.palTable.getEntry(this.vramMem[0x3f10 + i] & 32);
      }
    }
  }
  ...
  patternWrite(address, value) {
    ...
    if (leftOver < 8) {
      this
          .ptTile[tileIndex]
          .setScanline(leftOver, value, this.vramMem[address + 8]);
    } else {
      this
          .ptTile[tileIndex]
          .setScanline(leftOver - 8, this.vramMem[address - 8], value);
    }
  }
...
class NameTable {
  ...
  List tile;
  List attrib;
  NameTable(width, height, name) {
    this.width = width;
    this.height = height;
    this.name = name;

    this.tile = new List(width * height);
    this.attrib = new List(width * height);
...
class PaletteTable {
  ...
  makeTables() {
    ...
    for (var emph = 0; emph < 8; emph++) {
      ...
      this.emphTable[emph] = new List(64);
