...
class Mappers0 {
  ...
  loadROM() {
    if (!this.nes.rom.valid || this.nes.rom.romCount < 1) {
      throw "NoMapper: Invalid ROM! Unable to load.";
    }
  ...
  loadRomBank(bank, address) {
    ...
    utils.copyArrayElements(
        this.nes.rom.rom[bank], 0, this.nes.cpu.mem, address, 16384);
  }
  ...
  loadVromBank(bank, address) {
    ...
    utils.copyArrayElements(this.nes.rom.vrom[bank % this.nes.rom.vromCount], 0,
        this.nes.ppu.vramMem, address, 4096);

    var vromTile = this.nes.rom.vromTile[bank % this.nes.rom.vromCount];
    utils.copyArrayElements(
        vromTile, 0, this.nes.ppu.ptTile, address >> 4, 256);
  }
  ...
  load8kVromBank(bank4kStart, address) {
    ...
    this.loadVromBank(
        (bank4kStart + 1) % this.nes.rom.vromCount, address + 4096);
  }
  ...
  load1kVromBank(bank1k, address) {
    ...
    var bank4k = (bank1k / 4) % this.nes.rom.vromCount;
    var bankoffset = (bank1k % 4) * 1024;
    utils.copyArrayElements(
        this.nes.rom.vrom[bank4k], 0, this.nes.ppu.vramMem, bankoffset, 1024);
  ...
  load8kRomBank(bank8k, address) {
    ...
    utils.copyArrayElements(
        this.nes.rom.rom[bank16k], offset, this.nes.cpu.mem, address, 8192);
  }
...
// class Mappers1 extends Mappers0 {
//   dynamic nes;
//   Mappers1(this.nes);
...
// Mappers[1].prototype.getRegNumber = address => {
...
// // eslint-disable-next-line no-unused-vars
// Mappers[1].prototype.switchLowHighPrgRom = oldSetting => {
//   // not yet.
// };
...
// Mappers[1].prototype.switch32to16 = () => {
//   // not yet.
// };
...
// /**
//  * Mapper007 (AxROM)
//  * @example Battvaroads, Time Lord, Marble Madness
